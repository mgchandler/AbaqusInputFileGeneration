...
def write_input(settings):
    ...
    #If desired, create PML
    if 'PML' in settings['mesh']:
        PML_idxs = np.array(settings['mesh']['PML']['id']) + probe_coords.shape[1] + 1
        N_PML = len(PML_idxs)
    ...
    with open(filename, 'w') as f:
        ...
        f.write('{} 1\n'.format(ext_corners.shape[1]+N_PML-1)) #number of segments, number of boundary markers
        count = 1
        ...
        for ii in range(N_hole):
            ...
            count += 1
        if 'PML' in settings['mesh']:
            for ii in range(N_PML-1):
                f.write('{}    {} {} {}\n'.format(count, PML_idxs[ii], PML_idxs[ii+1], 3))
                count += 1
        f.write('{}\n'.format(len(xS)))
        if len(xS) >= 1:
            for hole in range(len(xS)):
    ...
    if 'PML' in settings['mesh']:
        ...
        PML_elements = np.zeros((n_elements))
        PML_loc = ext_corners[:, PML_idxs[0]-1]
        count = 0
        for element in range(n_elements):
            ...
            #Check whether this element is in the PML.
            PML_ticker = 0
            for node in range(3):
                ...
                if node_pos[0] - PML_loc[0] < 3e-16:
                    PML_ticker += 1
            # If all three nodes in the element are positioned behind the PML boundary
            if PML_ticker == 3:
                PML_elements[count] = int(line[0])
                count += 1
        PML_elements = np.array([item for item in PML_elements if item != 0], dtype=np.int)
    ...
    for el in range(num_els):
        ...
        with open(input_name, 'w') as i:
            ...
            # Create element set for the PML
            if 'PML' in settings['mesh']:
                i.write('*Elset, elset=All_but_PML\n')
                for a in range(1, n_elements+1):
                    if a not in PML_elements:
                        i.write('{},\n'.format(a))
                i.write('*Elset, elset=PML\n')
                for a in PML_elements:
                    i.write('{}, \n'.format(a))
            
            #Write the section definition
            # if 'PML' not in settings['mesh']:
            i.write('*Solid Section, elset=All_elements, material={}\n'.format(material_name))
            i.write(',\n')
            # else:
            #     i.write('*Solid Section, elset=All_but_PML, material={}\n'.format(material_name))
            #     i.write(',\n')
                # i.write('*Solid Section, elset=PML\n')
                # i.write('REF NODE={}, material={}'.format(1, material_name))
            ...
            #Write the step
            i.write('*Step, name=Step-1, inc=2000\n')
            i.write('*Dynamic, NOHAF, direct user control\n')
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        yaml_name = 'L_Vis.yml'
        os.chdir('C:\\Users\\mc16535\\OneDrive - University of Bristol\\Documents\\Postgrad\\Coding\\Abaqus\\FMC Generation\\v4')
